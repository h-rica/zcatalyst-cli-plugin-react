...
class ViteAdapter extends BaseBuildAdapter {
  ...
  async validate(command, cliVersion) {
    ...
    const hasViteConfig =
      fs.existsSync(viteConfigJs) || fs.existsSync(viteConfigTs);
    ...
    if (
      cliVersion &&
      !semver.satisfies(cliVersion, packageJson.compatibility["zcatalyst-cli"])
    ) {
  ...
  async build(packageJsonFile) {
    ...
    const buildReporter = require("../utils/build-reporter");
    const previousFileSizes = await buildReporter.measureFileSizesBeforeBuild(
      this.paths.appBuild
    );
    ...
    const viteBinCmd =
      process.platform === "win32" ? `${viteBin}.cmd` : viteBin;
    ...
    return new Promise((resolve, reject) => {
      ...
      buildProcess.on("exit", async (code) => {
        ...
        try {
          ...
          await buildReporter.printFileSizesAfterBuild(
            this.paths.appBuild,
            previousFileSizes,
            buildReporter.WARN_AFTER_BUNDLE_GZIP_SIZE,
            buildReporter.WARN_AFTER_CHUNK_GZIP_SIZE
          );
          log(" ");
   ...
   */
  async start(httpPort, masterPort, _watchMode) {
    // Load environment variables
    require("../config/env").loadClientEnv();
    ...
    viteProcess.on("error", (err) => {
      if (err.code === "ENOENT") {
        ...
        log(
          chalk.yellow(
            "Vite may not be installed. Run: npm install vite --save-dev"
          )
        );
      } else {
